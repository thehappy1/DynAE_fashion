...
def ae_constructor(encoder, decoder, dims, visualisation_dir):
    ...
    plot_model(autoencoder, show_shapes=True, show_layer_names=True,
               to_file=visualisation_dir + '/graph/FcAutoencoder.png')
    return autoencoder
...
def critic_constructor(dims, visualisation_dir, act='relu'):
    ...
    # hidden layer
    h = Dense(dims[-1], kernel_initializer=init, name='critic_%d' % (n_stacks - 1))(
        h)  # hidden layer, features are extracted from here  
...
class DynAE:
    ...
    def __init__(self, batch_size, dataset, dims, loss_weight, gamma=1.0, n_clusters=10, alpha=1.0,
                 visualisation_dir=PATH_VIS, ws=0.1, hs=0.1, rot=10, scale=0.0):
        ...
        self.datagen = ImageDataGenerator(width_shift_range=ws, height_shift_range=hs, rotation_range=rot,
                                          zoom_range=scale)
        self.ws = ws
        self.hs = hs
        self.rot = rot
        ...
        self.aci_ae = aci_ae_constructor(self.critic, self.ae, self.i_ae, self.loss_weight, self.dims,
    ...
    def train_aci_ae(self, x, y=None, maxiter=120e3, batch_size=256, validate_interval=2800, save_interval=2800,
                     save_dir=PATH_RESULT, verbose=1, aug_train=True):
        ...
        for ite in range(int(maxiter)):
            ...
            if ite % validate_interval == 0:
                if y is not None and verbose > 0:
                    ...
                    print('Iter %d: acc=%.5f, nmi=%.5f, loss_aci_ae=%.5f, loss_disc=%.5f' % (
                    ite, acc, nmi, avg_loss_aci_ae, avg_loss_disc))
            ...
            x_batch = self.random_transform(x_batch, ws=self.ws, hs=self.hs, rot=self.rot,
                                            scale=self.scale) if aug_train else x_batch
            ...
            loss2 = self.train_on_batch_disc(x_batch_recons_interp, x_batch_mix, alpha_interp,
                                             np.zeros((x_batch.shape[0],)))
    ...
    def train_dynAE(self, x, y=None, kappa=3, n_clusters=10, maxiter=1e5, batch_size=256, tol=1e-2,
                    validate_interval=140, show_interval=None, save_interval=2800, save_dir=PATH_RESULT,
                    aug_train=True):
        # init
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        logfile = open(save_dir + '/' + self.dataset + '/cluster/train_dynAE_gamma=' + str(self.gamma) + '_log.csv',
                       'w')
        ...
        logwriter = csv.DictWriter(logfile,
                                   fieldnames=['iter', 'acc', 'nmi', 'ari', 'acc_unconf', 'nmi_unconf', 'acc_conf',
                                               'nmi_conf', 'nb_unconf', 'nb_conf', 'loss'])
        logwriter.writeheader()
        ...
        for ite in range(int(maxiter)):
            if ite % validate_interval == 0:
                ...
                if y is not None:
                    ...
                    y_encoder, y_autoencoder = generate_supervisory_signals(x_emb, x, centers_emb, centers_img, beta1,
                                                                            beta2)
                    ...
                    acc_unconf, nmi_unconf, acc_conf, nmi_conf = self.compute_acc_and_nmi_conflicted_data(x, y,
                                                                                                          centers_emb,
                                                                                                          beta1, beta2)
                    ...
                    logdict = dict(iter=ite, acc=acc, nmi=nmi, ari=ari, acc_unconf=acc_unconf, nmi_unconf=nmi_unconf,
                                   acc_conf=acc_conf, nmi_conf=nmi_conf, nb_unconf=nb_unconf, nb_conf=nb_conf,
                                   loss=avg_loss)
                    logwriter.writerow(logdict)
                    logfile.flush()
                    ...
                    print(
                        ...
                        ite, acc, nmi, ari, acc_unconf, nmi_unconf, acc_conf, nmi_conf, nb_unconf, nb_conf, avg_loss))
            ...
            if ite % save_interval == 0:
                ...
                np.save(
                    '/media/sda/fschmedes/DynAE/DynAE/DynAE/visualisation/embeddings/' + self.dataset + '/vis_' + str(
                        ite) + '.npy', {'z_2d': z_2d, 'centers_2d': centers_2d, 'y_pred': y1_pred})
            ...
            Y_encoder, Y_autoencoder = generate_supervisory_signals(X_emb, X_img, centers_emb, centers_img, beta1,
                                                                    beta2)
            X_transformed = self.random_transform(X_img, ws=self.ws, hs=self.hs, rot=self.rot,
                                                  scale=self.scale) if aug_train else X_img
    ...
    def validate(self, x, y, dataset):
        features = self.predict_encoder(x)
        km = KMeans(n_clusters=len(np.unique(y)), n_init=20, random_state=0)
        y_pred = km.fit_predict(features)
        ...
        s_dbw = S_Dbw(features, y_pred)
        s = silhouette_score(features, y_pred, metric='euclidean')
        metrics.compute_tsne(features, y_pred, dataset)
        print(' ' * 8 + '|==>  acc: %.4f,  nmi: %.4f,  db: %.4f,  s_dbw: %.4f,  s: %.4f, ari: %.4f  <==|' % (
        acc, nmi, db, s_dbw, s, ari))
